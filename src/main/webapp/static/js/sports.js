var products;
if(document.URL=='http://localhost:8080/ShoppingBee/static/html/sports.htm'){
$.getJSON("/ShoppingBee/category/sports",function(result){
	products=result;
});
}
else if(document.URL=='http://localhost:8080/ShoppingBee/static/html/sports.htm?filter=0'){
$.getJSON("/ShoppingBee/category/sports",function(result){
	products=result;
});
}
	...
	$.getJSON("/ShoppingBee/sports/"+filter,function(result){
	products=result;
	});
}
...
// ALL FUNCTIONS
		...
		function selectsports(i){
			var id =products[i].product_id-1;
			var div= '<a href="productdetails.htm?id='+id+'"><div id="main3" class="'+products[i].price+
