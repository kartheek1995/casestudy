var products;
if(document.URL=='http://localhost:8080/ShoppingBee/static/html/clothes.htm'){
$.getJSON("/ShoppingBee/category/clothing",function(result){
	products=result;
});
}
else if(document.URL=='http://localhost:8080/ShoppingBee/static/html/clothes.htm?filter=0'){
$.getJSON("/ShoppingBee/category/clothing",function(result){
	products=result;
});
}
	...
	$.getJSON("/ShoppingBee/clothing/"+filter,function(result){
	products=result;
	});
}
...
// ALL FUNCTIONS
		function selectcloth(i){
			var id = products[i].product_id-1;
			var div= '<a href="productdetails.htm?id='+id+'"><div id="main4" class="'+products[i].price+
